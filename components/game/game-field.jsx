...
function GameCell({ children, onClick }) {
  return (
    <button className="border border-slate-200 cursor-pointer -ml-px -mt-px flex items-center justify-center" onClick={onClick}>
      {children}
    </button>
  );
}
...
function GameGrid({ children }) {
  return <div className="grid grid-cols-[repeat(19,_30px)] grid-rows-[repeat(19,_30px)] pl-px pt-px mt-3">{children}</div>;
}
