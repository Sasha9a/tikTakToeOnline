...
export function UiModal({ width = 'md', className, children, isOpen = false, onClose }) {
  ...
  const modal = (
    <div className={clsx('fixed inset-0 bg-slate-900/60 backdrop-blur-sm py-10 overflow-y-auto', className)} onClick={handleClick}>
      <div
        data-id="modal"
        className={clsx(
          'bg-white rounded-lg min-h-[320px] relative flex flex-col',
          {
            md: 'max-w-[640px] mx-auto w-full',
            full: 'mx-16'
          }[width]
        )}
      >
        <button
          className="
            w-8 h-8 rounded flex items-center
